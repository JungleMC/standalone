...
type Recipe struct {
	ID          JungleTree.Identifier
	Group       JungleTree.Identifier
	...
	Type        JungleTree.Identifier  `json:"type"`
...
type Ingredient struct {
	...
	Item  JungleTree.Identifier `json:"item,omitempty"`
	Tag   JungleTree.Identifier `json:"tag,omitempty"`
}
...
func writeResult(v interface{}) []byte {
	...
	case reflect.String:
		itemType, ok := item.ByName(JungleTree.Identifier(v.(string)))
		if !ok {
			panic("failed to find item type")
		}
	...
	case reflect.Map:
		...
		itemType, ok := item.ByName(JungleTree.Identifier(vMap["item"].(string)))
		if !ok {
			panic("failed to find item type")
		}
	...
	for k, v := range v.Key {
		...
		if reflect.TypeOf(v).Kind() == reflect.Slice {
			...
			for _, valInner := range val {
				...
				if value["item"] != nil {
					result.Item = JungleTree.Identifier(value["item"].(string))
				}

				if value["tag"] != nil {
					result.Tag = JungleTree.Identifier(value["tag"].(string))
				}
				return result
			}
		...
		if reflect.TypeOf(v).Kind() == reflect.String {
			return Ingredient{
				Count: 1,
				Item:  JungleTree.Identifier(v.(string)),
			}
		}
	}
